...
import { Plus, BookOpen, ClipboardList, Settings, LogOut, Search, Bell, Loader2, History, Save } from 'lucide-react';
...
export default function EducatorDashboard({ educator, onSignOut }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'courses' | 'logs' | 'settings'>('overview');
  ...
  useEffect(() => {
    ...
    // Load sessions for all educator's assignments (filtered client-side)
    ...
    <div className="min-h-screen bg-stone-50 flex">
      ...
      <aside className="w-64 bg-white border-r border-stone-200 flex flex-col">
        ...
        <div className="p-4 border-t border-stone-100">
          ...
          <button 
            onClick={() => onSignOut ? onSignOut() : auth.signOut()}
                ...
                <div className="space-y-8">
                  {selectedAssignment ? (
                    <div>
                      ...
                      <StudentList assignment={selectedAssignment} onClose={() => setSelectedAssignment(null)} />
                    </div>
                  ) : (
                    <section>
                        ...
                        {assignments.length === 0 ? (
                          ...
                          assignments.map(a => (
                            <div key={a.id} onClick={() => setSelectedAssignment(a)}>
                              <AssignmentCard
                                assignment={a}
                                stats={sessionStats[a.id] || { total: 0, completed: 0 }}
                              />
                            </div>
                          ))
                        )}
                      ...
                      <div className="bg-white rounded-3xl border border-stone-200 overflow-hidden">
                        {assignments.filter(a => {
                          ...
                          <div className="divide-y divide-stone-100">
                            {assignments
                              .filter(a => sessionStats[a.id]?.total > sessionStats[a.id]?.completed)
                              .map(a => (
                                <div
                                  key={a.id}
              ...
              {activeTab === 'courses' && (
                selectedCourse ? (
                  <CourseDetail
                    ...
                    onSelectAssignment={(a) => { setSelectedCourse(null); setSelectedAssignment(a); setActiveTab('overview'); }}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {courses.map(c => (
                        <div key={c.id} onClick={() => setSelectedCourse(c)}
                          ...
                          <h3 className="text-lg font-medium text-stone-900 mb-1 group-hover:text-emerald-700 transition-colors">{c.name}</h3>
                          <p className="text-sm text-stone-500 mb-3">
                            {assignments.filter(a => a.courseId === c.id).length} assignments
                            {c.contextFiles?.length ? ` · ${c.contextFiles.length} context file${c.contextFiles.length > 1 ? 's' : ''}` : ''}
                          </p>
...
function AssignmentCard({ assignment, stats }: {
  ...
  const dueLabel = dueDate
    ? dueDate < now
      ? 'Closed'
      : `Closes ${dueDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}`
    : 'No deadline set';
  ...
  return (
    <div className="bg-white p-6 rounded-3xl border border-stone-200 hover:shadow-lg transition-all cursor-pointer group">
      <div className="flex justify-between items-start mb-4">
        <h3 className="font-medium text-stone-900 group-hover:text-emerald-700 transition-colors pr-2">{assignment.title}</h3>
        <span className={cn('px-2 py-1 text-[10px] font-bold uppercase tracking-wider rounded-md whitespace-nowrap', statusColor)}>
          {assignment.status}
        </span>
      </div>
...
function SettingsPanel({ educatorId }: { educatorId: string }) {
  ...
  const seedTestData = async () => {
    ...
    try {
      ...
      const assignmentRef = await addDoc(collection(db, 'assignments'), {
        title: 'Test: Sample Interview Assignment',
  ...
  const handleSave = () => {
    const settings = { defaultCaptureMode, defaultQuestionCount, defaultResponseTime, testingMode };
    ...
    <div className="max-w-2xl space-y-8">
      ...
      <section className="bg-white rounded-3xl border border-stone-200 p-6 space-y-6">
        ...
        <div>
          ...
          <div className="flex bg-stone-100 p-1 rounded-xl w-48">
            {(['Snapshot', 'Video'] as const).map(m => (
              <button
                key={m}
                onClick={() => setDefaultCaptureMode(m)}
                className={cn(
        ...
        <div>
          ...
          <input
            type="range" min="8" max="16" value={defaultQuestionCount}
        ...
        <div>
          ...
          <input
            type="range" min="30" max="180" step="15" value={defaultResponseTime}
      ...
      <section className="bg-white rounded-3xl border border-stone-200 p-6 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-stone-400 mt-0.5">Reduces timers to 5 seconds and uses synthetic test data. Do not enable in production.</p>
          </div>
          <button
            onClick={() => setTestingMode(v => !v)}
            className={cn(
              'relative w-12 h-6 rounded-full transition-colors',
              testingMode ? 'bg-amber-500' : 'bg-stone-200'
            )}
          >
            <span className={cn(
              'absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all',
              testingMode ? 'left-7' : 'left-1'
            )} />
          </button>
        </div>
      ...
      <section className="bg-white rounded-3xl border border-stone-200 p-6 space-y-4">
        ...
        <p className="text-xs text-stone-500 leading-relaxed">
          Creates a "Test Interview Class" with a 3-question assignment and generates invite links for the 3 test accounts
          (abadawi@zewailcity.edu.eg, a.badawi@upm.edu.sa, ashraf.badawi@gmail.com).
          Links are valid for 7 days and single-use.
        </p>
        ...
        {seedLinks ? (
          <div className="space-y-3">
            ...
            <p className="text-xs text-stone-400">Click each URL field to select, then copy. Links appear in the Overview tab as "Test Interview Class".</p>
          </div>
        ) : (
          <button
            onClick={seedTestData}
            disabled={seeding}
            className="flex items-center gap-2 bg-stone-700 text-white px-5 py-2.5 rounded-2xl text-sm font-medium hover:bg-stone-600 disabled:opacity-50 transition-colors"
          >
